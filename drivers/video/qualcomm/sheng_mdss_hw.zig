...
/// Kconfig, reachable at comptime. Generated by scripts/Makefile.zig from
/// CONFIG_VIDEO_SHENG_MDSS_DEBUG.
///
/// The instrumentation below is compiled out entirely when this is false,
/// matching what sheng_mdss_debug.h has always done on the C side. It
/// used to be gated at RUNTIME by g_bb_on, which is set only by
/// sheng_bb_init() -- called only from sheng_mdss_debug.c, which is not
/// built at all in that configuration. So every blackbox call site, and
/// every string literal feeding one, was dead weight in the shipping
/// image while still costing a load and a branch on the boot path.
...
fn bbByte(c: u8) void {
    if (!debug_enabled or !g_bb_on) return;
...
export fn sheng_mdss_gdsc_probe(dispcc_base: usize, slot: u32) callconv(.c) void {
    if (slot >= 3) return;
    g_ctx.gdsc_probe[slot] = mmioRead32(dispcc_base, MDSS_GDSC_OFFSET);
}


export fn sheng_mdss_gdsc_probe_result() callconv(.c) i64 {
    return (@as(i64, g_ctx.gdsc_probe[0] >> 16) << 32) |
        (@as(i64, g_ctx.gdsc_probe[1] >> 16) << 16) |
        @as(i64, g_ctx.gdsc_probe[2] >> 16);
}
...
export fn sheng_mdss_gdsc_disable(dispcc_base: usize) callconv(.c) void {
    ...
    while (waited < GDSC_COLLAPSE_TIMEOUT_US) : (waited += 1) {
        if ((mmioRead32(dispcc_base, MDSS_GDSC_OFFSET) & GDSC_PWR_ON) == 0) {
            g_ctx.gdsc_collapse_us = waited;
            break;
        }
        udelay(1);
    } else {
        g_ctx.gdsc_collapse_us = 0xffffffff;
    }
...
fn dsiPhyPllRecommit() void {
    if (g_ctx.phy_base[0] == 0) return;
    mmioSetBits32(g_ctx.phy_base[0], CMN_CTRL_0, CTRL_0_PLL_SHUTDOWNB);
    mmioWrite32(g_ctx.phy_base[0] + PLL_BASE_OFFSET, PLL_SYSTEM_MUXES, 0xc0);
    dsiPhyPllCommitRate(g_ctx.phy_base[0]);
    dsiPhyPllConfigHzIndep(g_ctx.phy_base[0]);
    if (g_ctx.phy_base[1] != 0)
        mmioWrite32(g_ctx.phy_base[1] + PLL_BASE_OFFSET, PLL_PERF_OPTIMIZE, 0x22);
}
...
export fn sheng_mdss_dsi_phy_init(dsi_phy_base: usize, role: DsiPhyRole) callconv(.c) c_int {
    ...
    if (is_master) g_ctx.phy_base[0] = dsi_phy_base else g_ctx.phy_base[1] = dsi_phy_base;
...
fn dsiCmdDmaWait(dsi_base: usize) c_int {
    ...
    if (debug_enabled and g_timeout_logged < 6) {
...
///
/// g_ctx.dsi.retries counts transfers that needed a retry but eventually
...
export fn sheng_mdss_dsi_init_fail_info() callconv(.c) i64 {
    return (@as(i64, g_ctx.dsi.retries) << 32) |
        (@as(i64, @as(u16, @bitCast(@as(i16, @truncate(g_ctx.dsi.fail_idx))))) << 16) |
        @as(i64, @as(u16, @bitCast(@as(i16, @truncate(g_ctx.dsi.fail_rc)))));
}
...
fn dsiCmdDmaTxDualOnce(dsi0_base: usize, dsi1_base: usize, dma_addr: usize, len: usize) c_int {
    ...
    if (debug_enabled and g_cmd_trace_n == 0) {
...
export fn sheng_mdss_dsi_panel_init(dsi0_base: usize, dsi1_base: usize, dma_scratch: usize, dsc: DscMode) callconv(.c) c_int {
    ...
    for (nt36532e_init_sequence) |entry| {
        ...
        if (ret != 0) {
            ...
            g_ctx.dsi.fail_rc = ret;
            g_ctx.dsi.fail_idx = idx;
            return -(10000 + idx);
        }
...
fn smmuBypassMdssStream() void {
    ...
    g_ctx.smmu_cb_base = cbx_base; // for sheng_mdss_smmu_fault_diag()
...
export fn sheng_mdss_smmu_fault_diag() callconv(.c) i64 {
    if (g_ctx.smmu_cb_base == 0) return -1;
    const fsr = mmioRead32(g_ctx.smmu_cb_base, 0x058);
...
export fn sheng_mdss_smmu_fault_addr() callconv(.c) i64 {
    if (g_ctx.smmu_cb_base == 0) return -1;
    const far_lo = mmioRead32(g_ctx.smmu_cb_base, 0x060);
    const far_hi = mmioRead32(g_ctx.smmu_cb_base, 0x064);
...
/// Offset of struct sheng_blackbox within the pre-console reservation.
/// MUST match SHENG_BLACKBOX_OFFSET in include/sheng_mdss_abi.h; the
/// stage array is the first member there.
const BLACKBOX_OFFSET: usize = 0x3000;
/// The base comes from Kconfig via the generated config module rather
/// than being copied here. It used to be the literal 0x81200000 with
/// "CONFIG_PRE_CON_BUF_ADDR" in a trailing comment, so moving the
/// reservation would have left the Zig silently writing to the old
/// address while every C reader followed the new one.
const STATUS_ADDR: usize = @import("config").pre_con_buf_addr + BLACKBOX_OFFSET;
...
/// Record one bring-up stage. ONLY THE FIRST PROBE WRITES.
///
/// stdio_add_devices() probes the video device and qcom_late_init()
...
/// activated, so the second call reruns everything and used to overwrite
/// all eleven slots with a fresh record. A failed boot then reported
/// clean, which is precisely backwards: the first attempt's account is
/// the one that says why the panel is broken.
///
/// probe_count in the diag string reports that a retry happened, and
/// panel_init_ret (as opposed to panel_init_ret_first) carries the later
/// attempt's result, so nothing is lost by keeping the first record.
fn shengStage(stage: u32, ret: c_int) void {
    if (g_ctx.diag.probe_count > 1) return;
...
fn shengTmark(name: [*:0]const u8) void {
    if (g_ctx.tmark.count >= TMARK_MAX) return;
    g_ctx.tmark.name[g_ctx.tmark.count] = name;
    g_ctx.tmark.us[g_ctx.tmark.count] = timer_get_us();
    g_ctx.tmark.count += 1;
}
...
export fn sheng_mdss_tmark_get(i: u32, us: *c_ulong) callconv(.c) ?[*:0]const u8 {
    if (i >= g_ctx.tmark.count) return null;
    us.* = g_ctx.tmark.us[i];
    return g_ctx.tmark.name[i];
}

export fn sheng_mdss_tmark_count() callconv(.c) u32 {
    return g_ctx.tmark.count;
}
...
/// Boot-timing marks. Absolute microseconds since power-on, from the ARM
/// generic counter, which nothing in the boot chain resets.
///
/// 32, not 16: THE PROBE CAN RUN MORE THAN ONCE. A single bring-up takes
/// twelve marks, so at 16 a second probe silently truncated its record
/// after four -- which is how the retry went unnoticed in the first
/// place. A stale comment in sheng_mdss.c claimed this had been raised
/// already; it had been raised on a dead C copy of the constant, not on
/// this one.
const TMARK_MAX: u32 = 32;

const TmarkLog = struct {
    us: [TMARK_MAX]c_ulong = .{0} ** TMARK_MAX,
    name: [TMARK_MAX]?[*:0]const u8 = .{null} ** TMARK_MAX,
    count: u32 = 0,
};

/// DSI command-path outcomes, relayed in the diag string.
const DsiState = struct {
    retries: u32 = 0,
    fail_rc: c_int = 0,
    fail_idx: i32 = -1,
};

/// Bring-up state that outlives a single function.
///
/// Grouped rather than left as loose file-scope globals so the lifetimes
/// are legible: hardware topology discovered during bring-up, per-probe
/// results relayed to Linux, and the timing log. `sheng_mdss_bringup()`
/// is the only writer of probe_count and the only thing that decides what
/// a second probe may touch.
///
/// The SMMU page tables are deliberately NOT here: they are a DMA-visible
/// memory resource with a 4096-byte alignment requirement, not state.
const ShengDisplayCtx = struct {
    /// Set by sheng_mdss_dsi_phy_init() as each PHY comes up; indexed by
    /// DsiPhyRole. Every other function still takes its MMIO base from
    /// the caller -- these exist only because the dual-PHY start needs
    /// both after the fact.
    phy_base: [2]usize = .{ 0, 0 },
    smmu_cb_base: usize = 0,
...
export fn sheng_mdss_bringup(splash_live: c_int, fb: *ShengFb) callconv(.c) c_int {
    ...
    g_ctx.diag.panel_pm_pre = sheng_mdss_dsi_read_power_mode_single(DSI0, DSI_DMA_SCRATCH);
    ...
    // happened.
    const pm_pre = g_ctx.diag.panel_pm_pre;
    const quiet = (pm_pre & 0xff) == 0x08 and ((pm_pre >> 16) & 0xffff) == 0x01f7;
    g_ctx.diag.fastpath_state_ok = @intFromBool(quiet);
    g_ctx.diag.fastpath = g_ctx.diag.fastpath_state_ok;
    ...
    g_ctx.diag.panel_init_ret = ret;
    if (g_ctx.diag.panel_init_ret_first == 0x7fffffff) g_ctx.diag.panel_init_ret_first = ret;
    ...
    g_ctx.diag.status0 = mmioRead32(DSI0, DSI_STATUS0);
    g_ctx.diag.fifo = mmioRead32(DSI0, DSI_FIFO_STATUS);
    g_ctx.diag.lane = mmioRead32(DSI0, DSI_LANE_STATUS);
    g_ctx.diag.ackerr = mmioRead32(DSI0, DSI_ACK_ERR_STATUS);
    g_ctx.diag.timeout = mmioRead32(DSI0, DSI_TIMEOUT_STATUS);
    g_ctx.diag.pll_l = mmioRead32(DISPCC, PLL_L_VAL_OFF);
    g_ctx.diag.frames_early = mmioRead32(intf1, INTF_FRAME_COUNT);
    mdelay(FRAME_COUNT_SAMPLE_GAP_MS);
    g_ctx.diag.frames_late = mmioRead32(intf1, INTF_FRAME_COUNT);
    g_ctx.diag.fifo_late = mmioRead32(DSI0, DSI_FIFO_STATUS);

    g_ctx.diag.panel_pm = sheng_mdss_dsi_read_power_mode_single(DSI0, DSI_DMA_SCRATCH);
